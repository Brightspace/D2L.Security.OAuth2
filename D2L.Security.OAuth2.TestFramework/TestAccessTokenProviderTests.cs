...
namespace D2L.Security.OAuth2.TestFramework {
	...
	internal sealed class TestAccessTokenProviderTests {
		...
		private const string AUTH_URL = "https://auth-dev.proddev.d2l/core";

		[Test]
		public async void TestTestAccessTokenProvider() {
			using (var httpClient = new HttpClient()) {
				IAccessTokenProvider provider = TestAccessTokenProviderFactory.Create( httpClient, AUTH_URL );
				IAccessToken token = await provider.ProvisionAccessTokenAsync( new ClaimSet( "ExpandoClient", Guid.NewGuid() ), new[] { new Scope( "*", "*", "*" ) } );

				IAccessTokenValidator validator = AccessTokenValidatorFactory.CreateRemoteValidator( httpClient, new Uri( AUTH_URL ) );
