...
namespace D2L.Security.OAuth2.Tests.Integration.Validation {
	...
	internal sealed partial class AccessTokenValidatorTests {
		...
		internal sealed class EcDsa {
			...
			public async Task ValidateAsync_GoodSignature_Succeeds_WebCrypto( string jwk, string token ) {
				string host;
				...
				IValidationResponse response = await AccessTokenValidatorFactory
				...
				Assert.AreEqual( ValidationStatus.Expired, response.Status );
			}
		}
	}
}
